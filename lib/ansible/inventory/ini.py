                    ...
                    if len(tokens) > 1:
                        for t in tokens[1:]:
                            ...
                            try:
                                ...
                                raise errors.AnsibleError("{0!s}:{1!s}: Invalid ini entry: {2!s} - {3!s}".format(self.filename, lineno + 1, t, str(e)))
        ...
        for lineno in range(len(self.lines)):
            ...
            if line.startswith("[") and ":children]" in line:
                ...
                if kid_group is None:
                    raise errors.AnsibleError("{0!s}:{1:d}: child group is not defined: ({2!s})".format(self.filename, lineno + 1, line))
                else:
    ...
    def _parse_group_variables(self):
        ...
        for lineno in range(len(self.lines)):
            ...
            if line.startswith("[") and ":vars]" in line:
                ...
                group = self.groups.get(line, None)
                if group is None:
                    raise errors.AnsibleError("{0!s}:{1:d}: can't add vars to undefined group: {2!s}".format(self.filename, lineno + 1, line))
            elif line.startswith("#") or line.startswith(";"):
                pass
            elif line.startswith("["):
                group = None
                ...
                if "=" not in line:
                    raise errors.AnsibleError("{0!s}:{1:d}: variables assigned to group must be in key=value form".format(self.filename, lineno + 1))
                else:
