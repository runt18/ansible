    ...
    def run(self, terms, inject=None, **kwargs):
        ...
        for item0 in elementlist:
            if not isinstance(item0, dict):
                raise errors.AnsibleError("subelements lookup expects a dictionary, got '{0!s}'".format(item0))
            if item0.get('skipped',False) != False:
                # this particular item is to be skipped
                continue 
            if not subelement in item0:
                raise errors.AnsibleError("could not find '{0!s}' key in iterated item '{1!s}'".format(subelement, item0))
            if not isinstance(item0[subelement], list):
                raise errors.AnsibleError("the key {0!s} should point to a list, got '{1!s}'".format(subelement, item0[subelement]))
