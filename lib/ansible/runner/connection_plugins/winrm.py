...
class Connection(object):
    ...
    def _winrm_connect(self):
        ...
        vvv("ESTABLISH WINRM CONNECTION FOR USER: {0!s} on PORT {1!s} TO {2!s}".format(self.user, port, self.host), host=self.host)
        netloc = '{0!s}:{1:d}'.format(self.host, port)
        exc = None
        for transport, scheme in self.transport_schemes['http' if port == 5985 else 'https']:
            ...
            vvvv('WINRM CONNECT: transport={0!s} endpoint={1!s}'.format(transport, endpoint),
                 host=self.host)
            ...
            try:
                ...
                if m:
                    ...
                    if code == 401:
                        ...
                        return protocol
                vvvv('WINRM CONNECTION ERROR: {0!s}'.format(err_msg), host=self.host)
                continue
        if exc:
    ...
    def _winrm_exec(self, command, args=(), from_exec=False):
        if from_exec:
            vvvv("WINRM EXEC {0!r} {1!r}".format(command, args), host=self.host)
        else:
            vvvvv("WINRM EXEC {0!r} {1!r}".format(command, args), host=self.host)
        ...
        try:
            ...
            if from_exec:
                vvvv('WINRM RESULT {0!r}'.format(response), host=self.host)
            else:
                vvvvv('WINRM RESULT {0!r}'.format(response), host=self.host)
            vvvvv('WINRM STDOUT {0!s}'.format(response.std_out), host=self.host)
            vvvvv('WINRM STDERR {0!s}'.format(response.std_err), host=self.host)
            return response
        finally:
            if command_id:
    ...
    def exec_command(self, cmd, tmp_path, become_user=None, sudoable=False, executable=None, in_data=None):
        ...
        if '-EncodedCommand' in cmd_parts:
            ...
            vvv("EXEC {0!s}".format(decoded_cmd), host=self.host)
        else:
            vvv("EXEC {0!s}".format(cmd), host=self.host)
        ...
        try:
            ...
            traceback.print_exc()
            raise errors.AnsibleError("failed to exec cmd {0!s}".format(cmd))
        ...
        with open(in_path) as in_file:
            ...
            for offset in xrange(0, in_size, buffer_size):
                try:
                    ...
                    vvvv("WINRM PUT {0!s} to {1!s} (offset={2:d} size={3:d})".format(in_path, out_path, offset, len(out_data)), host=self.host)
                    cmd_parts = powershell._encode_script(script, as_list=True)
        ...
        try:
            ...
            while True:
                try:
                    ...
                    '''.format(**dict(buffer_size=buffer_size, path=powershell._escape(in_path), offset=offset))
                    vvvv("WINRM FETCH {0!s} to {1!s} (offset={2:d})".format(in_path, out_path, offset), host=self.host)
                    cmd_parts = powershell._encode_script(script, as_list=True)
                    result = self._winrm_exec(cmd_parts[0], cmd_parts[1:])
                    if result.status_code != 0:
                        raise IOError(result.std_err.encode('utf-8'))
